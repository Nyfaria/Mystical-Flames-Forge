...
public class PhoenixDragon extends TamableAnimal implements Saddleable, FlyingAnimal, PlayerRideableJumping, IAnimatable {
    ...
    // data value IDs
    private static final EntityDataAccessor<String> DATA_BREED = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FLYING = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_SADDLED = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_AGE = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> IS_GOINGUP = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> IS_HOVERING = SynchedEntityData.defineId(PhoenixDragon.class, EntityDataSerializers.BOOLEAN);
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(this.isFlying()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("flight.phoenix_model", true));
            return PlayState.CONTINUE;
        }
        if(this.isVehicle() && event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("run.phoenix_model", true));
            return PlayState.CONTINUE;
        }
        if(event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("walk.phoenix_model", true));
            return PlayState.CONTINUE;
        }
        if(this.isInSittingPose() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("sit.phoenix_model", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("idle.phoenix_model", true));
        return PlayState.CONTINUE;
    }

    public PhoenixDragon(EntityType<? extends TamableAnimal> type, Level level) {
        super(type, level);
        this.maxUpStep = 1;
        this.noCulling = true;
        this.fireImmune();
        this.moveControl = new PhoenixMoveController(this);
        this.setTame(false);
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob p_146744_) {
        return ModEntities.PHOENIX.get().create(serverLevel);
    }
