...
public class EntityQueensCrown extends AbstractDragonBase {
    ...
    public EntityQueensCrown(EntityType<? extends TamableAnimal> p_21803_, Level p_21804_) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if ((isFlying() && isHovering())) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("hover.queenscrown_model", true)); // hover
            return PlayState.CONTINUE;
        }
        if (isFlying() && !isHovering()) {
            if (this.xRotO < -20 || isGoingUp()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("up.queenscrown_model", true)); //flyup
                return PlayState.CONTINUE;
            }
            if (this.xRotO < 8 && this.xRotO > -20) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("flight.queenscrown_model", true)); // fly
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 8 && this.xRotO < 12) { // < 20
                event.getController().setAnimation(new AnimationBuilder().addAnimation("glide.queenscrown_model", true)); // glide
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 12 && this.xRotO < 15) { // < 30
                event.getController().setAnimation(new AnimationBuilder().addAnimation("dive.queenscrown_model", true)); // glidedown
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 15) { // > 30
                event.getController().setAnimation(new AnimationBuilder().addAnimation("dive.queenscrown_model", true)); // dive
                return PlayState.CONTINUE;
            }
        }
        if (event.isMoving()) {
