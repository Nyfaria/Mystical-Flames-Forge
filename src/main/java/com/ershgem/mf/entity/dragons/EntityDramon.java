...
public class EntityDramon extends AbstractDragonBase {
    ...
    public EntityDramon(EntityType<? extends TamableAnimal> p_21803_, Level p_21804_) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if ((isFlying() && isHovering())) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("hover.Dramon_Model", true)); // hover
            return PlayState.CONTINUE;
        }
        if (isFlying() && !isHovering()) {
            if (this.xRotO < -20 || isGoingUp()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("up.Dramon_Model", true)); //flyup
                return PlayState.CONTINUE;
            }
            if (this.xRotO < 8 && this.xRotO > -20) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("flight.Dramon_Model", true)); // fly
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 8 && this.xRotO < 12) { // < 20
                event.getController().setAnimation(new AnimationBuilder().addAnimation("glide.Dramon_Model", true)); // glide
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 12 && this.xRotO < 15) { // < 30
                event.getController().setAnimation(new AnimationBuilder().addAnimation("dive.Dramon_Model", true)); // glidedown
                return PlayState.CONTINUE;
            }
            if (this.xRotO > 15) { // > 30
                event.getController().setAnimation(new AnimationBuilder().addAnimation("dive.Dramon_Model", true)); // dive
                return PlayState.CONTINUE;
            }
        }
        if (event.isMoving()) {
