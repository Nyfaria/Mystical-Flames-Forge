package com.ershgem.mf.entity.dragons.lycan;
...
public class LycanDragon extends TamableAnimal implements Saddleable, FlyingAnimal, PlayerRideableJumping, IAnimatable {
    ...
    // data value IDs
    private static final EntityDataAccessor<String> DATA_BREED = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FLYING = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_SADDLED = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_AGE = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> IS_GOINGUP = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> IS_HOVERING = SynchedEntityData.defineId(LycanDragon.class, EntityDataSerializers.BOOLEAN);
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(this.isFlying()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("flight.lycan_model", true));
            return PlayState.CONTINUE;
        }
        if(this.isVehicle() && event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("run.lycan_model", true));
            return PlayState.CONTINUE;
        }
        if(event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("walk.lycan_model", true));
            return PlayState.CONTINUE;
        }
        if(this.isInSittingPose() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("sit.lycan_model", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("idle.lycan_model", true));
        return PlayState.CONTINUE;
    }

    public LycanDragon(EntityType<? extends TamableAnimal> type, Level level) {
        super(type, level);
        this.maxUpStep = 1;
        this.noCulling = true;
        this.fireImmune();
        this.moveControl = new LycanMoveController(this);
        this.setTame(false);
    }
